...
- 控制台充值页（2026-09-18 截图核对）确认规则：自定义金额 **¥30 起、¥5000 封顶**
  （超出联系客服对公转账）；定额档位为 10/30/100/200/500 元（对应不同 recahrgeId，
  实测 22 = ¥10 档）；余额单位平台称为"模力"（1e-4 口径）
- **定额档位对照表**（2026-09-18 控制台抓包确认，下单参数须逐字节照抄）：
  ...
  `amounts` 单位按档位约为 1e-8 元；**SDK 必须按此表原样传参**（自行按比例
  换算未经验证）。自定义金额（recahrgeId=null）单位 1e-4，范围 30–5000。
- 已知未确认：微信渠道 `payWay` 取值、订单过期时长
