...
///   且不一定等于用户目录，可能导致 `.ssy-switch/cc-switch.db` 路径变化，从而“看起来像数据丢失”。
///
/// ## 测试隔离
///
...
/// 获取应用配置目录路径 (~/.ssy-switch)
// SSY-Switch: 数据目录与 CC Switch (.ssy-switch) 完全隔离，不做跨产品迁移；
// 上游 v3.10.3 的 Windows HOME 回退逻辑随之移除（它只针对 .ssy-switch 布局）。
pub fn get_app_config_dir() -> PathBuf {
    if let Some(custom) = crate::app_store::get_app_config_dir_override() {
        return custom;
    }

    get_home_dir().join(".ssy-switch")
}
