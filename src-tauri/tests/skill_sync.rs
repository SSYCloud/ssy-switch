...
fn import_from_apps_does_not_rewrite_selected_app_directory() {
    ...
    let ssot_skill_dir = home.join(".ssy-switch").join("skills").join("codex-skill");
...
fn sync_to_app_removes_disabled_and_orphaned_ssot_symlinks() {
    ...
    let ssot_dir = home.join(".ssy-switch").join("skills");
...
fn uninstall_skill_creates_backup_before_removing_ssot() {
    ...
    let ssot_skill_dir = home.join(".ssy-switch").join("skills").join("backup-skill");
