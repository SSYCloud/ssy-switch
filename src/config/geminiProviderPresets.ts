...
const ALL_geminiProviderPresets: GeminiProviderPreset[] = [
  {
    name: "Shengsuanyun",
...
const keepOfficialPreset = (p: (typeof ALL_geminiProviderPresets)[number]): boolean =>
  p.category === "official" ||
  p.name === "Shengsuanyun" ||
  p.partnerPromotionKey === "shengsuanyun" ||
  (p as { requiresOAuth?: boolean }).requiresOAuth === true;

export const geminiProviderPresets: GeminiProviderPreset[] = ALL_geminiProviderPresets.filter(keepOfficialPreset);
